...
public class FindKth {
    ...
    public static int findKth2(int[] arr, int k) {
        ...
        return process(newArr, 0, newArr.length - 1, k - 1);
    }
    ...
    public static int process(int[] arr, int left, int right, int index) {
        ...
        else if (index < range[0]) return process(arr, left, range[0] - 1, index);
        else return process(arr, range[1] + 1, right, index);
    }
