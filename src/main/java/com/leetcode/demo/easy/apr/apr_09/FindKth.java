...
public class FindKth {
    ...
    public static void main(String[] args) {
        int[] arr = {4, 7, 2, 1, 8, 9, 0, 11, 10};
        System.out.println(findKth1(arr, 6));
        System.out.println(findKth2(arr, 6));
        System.out.println(findKth3(arr, 6));
    }
    ...
    public static int bfprt(int[] arr, int left, int right, int index) {
        ...
        else return bfprt(arr, left, range[0] - 1, index);
    }
    ...
    public static int mediaOfMedias(int[] arr, int left, int right) {
        ...
        for (int i = 0; i < medias.length; i++) {
            medias[i] = computeMedia(arr, 5 * i + left, Math.min(right, 5 * i + 4 + left));
        }
