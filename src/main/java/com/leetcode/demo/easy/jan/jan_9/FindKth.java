...
public class FindKth {
    ...
    public static int process(int[] arr, int index, int left, int right) {
        ...
        int[] partitionRange = partition(arr, pivot, left, right);
        if (index >= partitionRange[0] && index <= partitionRange[1]) {
            return arr[index];
        } else if (partitionRange[0] > index) {
    ...
    public static int findKth3(int[] arr, int k) {
        ...
        return bfprt(newArr, k - 1, 0, newArr.length - 1);
    }
    ...
    public static int bfprt(int[] arr, int index, int left, int right) {
        ...
        else if (index < partitionRange[0]) return bfprt(arr, index, left, partitionRange[0] - 1);
        else return bfprt(arr, index, partitionRange[1] + 1, right);
    }
